...
#Variables, constants and map definition lenght
...
while len(pokemon_trainers) < AMOUNT_OF_TRAINERS:
    ...
    if new_trainer != player_gcs and new_trainer not in pokemon_trainers \
...
while not end_game:
    ...
    for coordinate_y in range(MAP_HEIGHT):
        ...
        for coordinate_x in range(MAP_WIDTH):
            ...
            if player_gcs[X_AXIS] == coordinate_x and player_gcs[Y_AXIS] == coordinate_y:
                char_to_draw = "{☺}"

            for trainer in pokemon_trainers:
                if trainer[X_AXIS] == coordinate_x and trainer[Y_AXIS] == coordinate_y:
                    char_to_draw = "{☻}"
                    if player_gcs[X_AXIS] == trainer[X_AXIS] and player_gcs[Y_AXIS] == trainer[Y_AXIS]:
                        char_to_draw = "☺↔☻"             #animation of versus
    ...
    if player_gcs in pokemon_trainers:
        print("¿Quieres enfrentarte a este entrenador nivel {}?".format
              (pokemon_trainers.index([player_gcs[X_AXIS], player_gcs[Y_AXIS]])+1+level_pikachu))
    ...
    if direction == "w":
        new_position = [player_gcs[X_AXIS], (player_gcs[Y_AXIS]-1) % MAP_HEIGHT]

    elif direction == "s":
        new_position = [player_gcs[X_AXIS], (player_gcs[Y_AXIS]+1) % MAP_HEIGHT]

    elif direction == "a":
        new_position = [(player_gcs[X_AXIS]-1) % MAP_WIDTH, player_gcs[Y_AXIS]]
        ...
        if player_gcs in pokemon_trainers:
            battle_number = pokemon_trainers.index([player_gcs[X_AXIS], player_gcs[Y_AXIS]]) + level_pikachu
            ...
            if win and level_pikachu == 3:
                ...
                level_pikachu += 1
                pokemon_trainers.remove([player_gcs[X_AXIS], player_gcs[Y_AXIS]])
                print("╔" + "═" * 84 + "╗")
                print("║" + "VICTORIA".center(84) + "║")
                print("║" + " " * 84 + "║")
                print("║" + "TU PIKACHU HA SUBIDO DE NIVEL".center(84) + "║")
                print("║" + " " * 84 + "║")
